...
class Paths {
    ...
    inline public static function inst(song:String, ?diff:String)
        return getPath('songs/$song/song/' + diff != null ? '$diff/' : '' + 'Inst.$SOUND_EXT');

    inline public static function voices(song:String, ?suffix:String = '', ?diff:String)
        return getPath('songs/$song/song/' + diff != null ? '$diff/' : '' + 'Voices$suffix.$SOUND_EXT');
}